...
export default class ProfileForm extends FormView {
  ...
  switchOn () {
    ...
    // después de mucha investigación no supe como m**** cambiar los
    // availableLocales a los que quería, así que hacemos esta chanchada
    //config.availableLocales.forEach((locale) => {
    ;['pt', 'es'].forEach((locale) => {
      //if (locale != 'pt' && locale != 'es') return
