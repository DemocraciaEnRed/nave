...
export default class HomeForum extends Component {
  ...
  render () {
    ...
    const tipoConsulta = forum.extra.contentType === 'llamado' && t("forum.type.llamado") || t("forum.type.consulta")
      ...
      <div className='ext-forum-home'>
            ...
            <div className='container'>
              ...
              { forum.extra.contentType === 'propuestas' &&
                <a
                  className='btn btn-primary'
                  onClick={this.handleScroll} >
                  {t("forum.page.action-propuestas")}
                </a>
              }
              { forum.extra.contentType === 'llamado' &&
                <a
                  className='btn btn-primary'
                  onClick={this.handleScroll} >
                   { !isClosed &&  t("forum.page.action-llamado.open") || t("forum.page.action-llamado.closed") }
                </a>
              }
            </div>
          </div>
            ...
            <div className='row'>
              <div className='col-md-8 offset-md-2'>
                <div className="consulta-cerrada">
                  ...
                  <h5>{ t("forum.page.closed", { forumType: tipoConsulta }) }</h5>
                  ...
                  {forum.extra.linkCierre &&
                    <a
                      ...
                      {t("forum.page.informe-cierre")}
                    </a>
                  }
                  <p>{ t("forum.page.no-contributions", { forumType: tipoConsulta }) }</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className='container topics-container' >
          {this.state.topics.length > 0 && (forum.extra.contentType === 'ejes' || forum.extra.contentType === undefined) &&
            <h5 id='anchor'>{this.state.topics.length} {t("forum.page.topics-subtitle.ejes")}</h5>
          }
          {this.state.topics.length > 0 && forum.extra.contentType === 'propuestas' &&
            <h5 id='anchor'>{this.state.topics.length} {t("forum.page.topics-subtitle.propuestas")}</h5>
          }
          {this.state.topics.length > 0 && forum.extra.contentType === 'llamado' &&
            <h5 id={!isClosed && forum.extra.contentType === 'llamado' ? '' : 'anchor'}>{this.state.topics.length} {t("forum.page.topics-subtitle.llamado")}</h5>
          }
