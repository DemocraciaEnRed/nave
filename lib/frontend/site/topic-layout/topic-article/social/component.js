...
export default ({ topic }) => {
  ...
  const twitterText = encodeURIComponent(
    config.tweetText ?
        t(config.tweetText, { organizationName: t("common.app-name"), bajadaPlataforma: t("homepage.presentation.slogan") }).replace(/\\n/g, "\n")
      : mediaTitle
  )
  ...
  return (
    <div className='topic-article-content topic-social'>
      <div className='participants-box'>
        <span className='paticipants-box-published'>{t("proposal-article.state.published")}</span>
        <span>{
        ...
        <span className={ topic.closed ? 'closed' : 'open'} >{topic.closed ? t("proposal-article.state.closed") : t("proposal-article.state.open")}</span>
        <span>{t("proposal-article.share")}</span>
      </div>
