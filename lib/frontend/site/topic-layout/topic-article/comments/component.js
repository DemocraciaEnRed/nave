...
export class Comments extends Component {
  ...
  render () {
    const { forum, topic, commentsFetch } = this.props

    const forumClosingDate = forum.extra && forum.extra.closingAt
    const topicClosingDate = topic.closingAt
    // respetar primero la fecha del topic, después la del forum
    const closingDate = topicClosingDate || forumClosingDate
    // si no hay fecha de cierre definida dejar comentarios abiertos
    const commentsClosed = closingDate ? new Date(closingDate) < new Date() : false
            ...
            <CommentsOrderBy onSort={this.props.handleSort} />
            {
              ...
              (
                <div className="alert alert-info" role="alert" style={{marginTop: '10px'}}>
                  ...
                  <span>{t('comments.closed.title')}</span>
                  <br /><span><Link to='/'>{t('comments.closed.message')}</Link></span>
                </div>
              )
            }
          </h2>
